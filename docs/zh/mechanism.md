...
**「上次」的单位是尝试，不是 LLM 调用（[已证明]，本机实测 2026-09-19）**：①②与冷却闸读的都是
**组织段尝试账**，而机械拍（没接执行者）跑组织段时同样写一行——一个只跑机械拍的新状态根，
拍 1 就留下了一条记录。后果是可操作的：在接了执行者的部署里「试过」与「调过 LLM」重合，
但在纯机械部署或**手动调试一拍**里，那一拍会占用冷却窗，把真正的 LLM 组织段推迟至多
`org_cooldown_min` 分钟。改成「只有调过 LLM 才算一次」属机制变更（要动写入条件＋测试＋
中英正本），未拍板前保持现状；本段只负责让读文本的人知道自己在读哪一种量。
