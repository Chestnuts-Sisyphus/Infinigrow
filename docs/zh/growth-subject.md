...
- **两个子树只有一边有容量判据**：`journal/` 由园丁按 `journal_keep_files`（默认 200 篇）
  ...
  「可移动」判据，不是先调数字。**该触发器现已成立（实测 2026-09-19，拍 745）：**
  `observation_composition` 把 20 格逐文件观测面读成 `app` 18 ／ `journal` 2（`app/` 现 201
  件、`journal/` 194 篇）。把主体拷进临时目录做对照可证其机制：把 `app/` 只留最新 4 件，同一
  个观测窗口立刻翻成 `journal` 16 ／ `app` 4——所以挤占来自 app 证据件「mtime 恒最新」，不是
  20 格太小。**结论：需拍板。** 解药是那条尚不存在的「证据件已结案、且不被任何成熟链引用」
  判据；观测面上限（10／20）本轮一律不动，抬它只会让更多证据件挤掉 `journal/`，不新增任何
  可对账信息。
