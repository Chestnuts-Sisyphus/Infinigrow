...
- **扫现实的对比窗口＝同一拍的「动手前 → 动手后」**（K14/N55）：组织会话的输入块
  「本拍现实变化」比的是 `subject-before.json`（步 2 写）与 `subject.json`（步 8 写）
  ——那正好是**上一手动作**造成的现实变化（这类变化在差异账里被 `act_caused` 中和，
  只有这块能看见）。**不许**拿「上一拍快照 vs 本拍动手前清单」比：两者之间没有任何
  动作，那块判据会**恒为空**（实测三次组织会话全是「无新出现无变化」＝K14 等于没接上）。
