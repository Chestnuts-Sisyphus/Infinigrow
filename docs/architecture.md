...
| 运行体 | 文件 | 它负责 | 它**不**负责 |
|---|---|---|---|
| **生长主体** | `engine/subject.py` | 说清「引擎在长什么」；机械观测（存在性/文件数/字节数，有界稳定排序） | 内容级判断（那是执行者与组织会话的事） |
...
| **域饱和 / 轮转** | `engine/domain_saturation.py`·`ledger/rotation.py` | 同域同量只养一根未完成芽；账本历史搬进归档（只移动不删） | 不做内容判断；不删任何行 |
