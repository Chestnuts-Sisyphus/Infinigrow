...
**这里量的单位是「一次组织段尝试」，不是「一次调成 LLM 的调用」**。写入方只有三条路径，全都
发生在组织段**真的起跑之后**：① 提示文件缺失（没碰执行者就返回）、② 执行者调用未成功
（rc≠0 或超时）、③ 正常跑完。②③ 都调过执行者，① 一次都没调——所以「试过」确实不等于
「调成过」。**但没接执行者时组织段根本不跑、也不记账**（`run_tick` 只在 `_make_runner` 给出
runner 时才进这一段）：本机实测（2026-09-19）`IG_EXECUTOR=""` 的新状态根连跑 3 拍之后
`org-llm.jsonl` **不存在**，`org-check` 仍报「①从未跑过组织段」——纯机械部署不会自我节流。
后果仍然可操作，只是条件变了：**接了执行者的那一拍**（本机 env 里就带着 `IG_EXECUTOR`，
手动拍一拍也算）会占住冷却窗，把下一次真正的 LLM 组织段推迟至多 `cooldown_min` 分钟。
**2026-09-19 定案：保持「尝试」口径，不改成「只有调成 LLM 才算」**——被推翻过一次的上文
（上一版本这里写着「只跑机械拍的根也会写一行」）就是没接执行者这一路的反面教材。留下的理由：
失败的那次（rc≠0／超时／提示词缺失）若不算一次尝试，冷却闸就挡不住每拍重试同一个坏掉的
执行者，①②③④ 会再次全部放行——那是拿 token 换一个已知失败的调用。
